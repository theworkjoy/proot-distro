...
##
## Originally created by Leonid Pliushch <sylirre@neoterm.dev> for NeoTerm
## project.
##
...
##
## 2. The PRoot-Distro is designed to work only in NeoTerm. Supporting other
...
##    ownership of NeoTerm environment directories and in some cases it also
...
##
##      * @NEOTERM_APP_PACKAGE@ - sets NeoTerm app package (io.neoterm).
##      * @NEOTERM_PREFIX@ - sets the installation prefix path.
##      * @NEOTERM_HOME@ - sets the NeoTerm home directory path.
##
...
DEFAULT_PATH_ENV="/usr/local/sbin:/usr/local/bin:/usr/sbin:/usr/bin:/sbin:/bin:/usr/local/games:/usr/games:@NEOTERM_PREFIX@/bin:/system/bin:/system/xbin"
...
# Disable neoterm-exec or other things which may interfere with proot.
# It is expected that all dependencies have fixed hardcoded paths according
# to NeoTerm file system layout.
unset LD_PRELOAD
...
command_install() {
	...
	if [ -f "${distro_plugin_script}" ]; then
		...
		# set to xterm-256color (NeoTerm app default).
		...
		echo "aid_$(id -un):x:$(id -u):$(id -g):NeoTerm:/:/sbin/nologin" >> \
...
run_proot_cmd() {
	...
	if [ "$DISTRO_ARCH" != "$DEVICE_CPU_ARCH" ]; then
		...
		case "$DISTRO_ARCH" in
			aarch64) cpu_emulator_path="@NEOTERM_PREFIX@/bin/qemu-aarch64";;
			arm)
				if [ "$DEVICE_CPU_ARCH" != "aarch64" ]; then
					cpu_emulator_path="@NEOTERM_PREFIX@/bin/qemu-arm"
				fi
				;;
			i686)
				if [ "$DEVICE_CPU_ARCH" != "x86_64" ]; then
					cpu_emulator_path="@NEOTERM_PREFIX@/bin/qemu-i386"
				fi
				;;
			x86_64)
				if [ "$PROOT_DISTRO_X64_EMULATOR" = "QEMU" ]; then
					cpu_emulator_path="@NEOTERM_PREFIX@/bin/qemu-x86_64"
				elif [ "$PROOT_DISTRO_X64_EMULATOR" = "BLINK" ]; then
					cpu_emulator_path="@NEOTERM_PREFIX@/bin/blink"
				else
					msg
	...
	if [ -n "$cpu_emulator_arg" ]; then
		...
		cpu_emulator_arg="${cpu_emulator_arg} --bind=@NEOTERM_PREFIX@"
...
setup_fake_proc() {
	...
	if [ ! -f "${INSTALLED_ROOTFS_DIR}/${distro_name}/proc/.version" ]; then
		...
		Linux version ${DEFAULT_FAKE_KERNEL_VERSION} (proot@neoterm) (gcc (GCC) 12.2.1 20230201, GNU ld (GNU Binutils) 2.40) #1 SMP PREEMPT_DYNAMIC Wed, 01 Mar 2023 00:00:00 +0000
		EOF
	fi
...
command_login() {
	...
	# to Android and NeoTerm so user can interact with host file system.
	if ! $isolated_environment; then
		...
		set -- "--bind=/data/data/@NEOTERM_APP_PACKAGE@/cache" "$@"
		if [ -d "/data/data/@NEOTERM_APP_PACKAGE@/files/apps" ]; then
			set -- "--bind=/data/data/@NEOTERM_APP_PACKAGE@/files/apps" "$@"
		fi
		set -- "--bind=@NEOTERM_HOME@" "$@"
	...
	if ! $isolated_environment || $need_cpu_emulator; then
		...
		set -- "--bind=@NEOTERM_PREFIX@" "$@"
	fi

	# Use NeoTerm home directory if requested.
	# Ignores --isolated.
	if $use_neoterm_home; then
		if [ "$login_user" = "root" ]; then
			set -- "--bind=@NEOTERM_HOME@:/root" "$@"
		else
			if [ -f "${INSTALLED_ROOTFS_DIR}/${distro_name}/etc/passwd" ]; then
				...
				set -- "--bind=@NEOTERM_HOME@:${user_home}" "$@"
			else
				set -- "--bind=@NEOTERM_HOME@:/home/${login_user}" "$@"
			fi
		fi
	fi
	...
	if $make_host_tmp_shared; then
		set -- "--bind=@NEOTERM_PREFIX@/tmp:/tmp" "$@"
	fi
...
command_login_help() {
	...
	msg
	msg "  ${GREEN}--neoterm-home        ${CYAN}- Mount NeoTerm home directory to /root.${RST}"
	msg "                         ${CYAN}Takes priority over '${GREEN}--isolated${CYAN}' option.${RST}"
	msg
	msg "  ${GREEN}--shared-tmp         ${CYAN}- Mount NeoTerm temp directory to /tmp.${RST}"
	msg "                         ${CYAN}Takes priority over '${GREEN}--isolated${CYAN}' option.${RST}"
	msg
	...
	msg "  ${CYAN}* ${YELLOW}/data/data/@NEOTERM_APP_PACKAGE@${RST}"
	msg "  ${CYAN}* ${YELLOW}/sdcard${RST}"
	msg "  ${CYAN}* ${YELLOW}/storage${RST}"
	msg "  ${CYAN}* ${YELLOW}/system${RST}"
	msg "  ${CYAN}* ${YELLOW}/vendor${RST}"
	msg
	msg "${CYAN}This should be enough to get NeoTerm utilities like neoterm-api or${RST}"
	msg "${CYAN}neoterm-open get working. If they do not work for some reason,${RST}"
	...
	msg "${CYAN}in NeoTerm sessions.${RST}"
	msg
...
show_version() {
	msg "${ICYAN}Proot-Distro v${PROGRAM_VERSION} by NeoTerm (@sylirre).${RST}"
}
...
if [ -x "@NEOTERM_PREFIX@/bin/dpkg" ]; then
	if [ "$DEVICE_CPU_ARCH" != "$("@NEOTERM_PREFIX@"/bin/dpkg --print-architecture)" ]; then
		msg
		msg "${BRED}Error: the CPU architecture reported by system does not match the architecture of NeoTerm packages. Do not attempt to hijack system properties by using 'linux32' or similar utilities.${RST}"
		msg
		exit 1
	fi
fi
