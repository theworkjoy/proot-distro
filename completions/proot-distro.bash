_ls_available_dists() {
	if [ ! -e "@NEOTERM_PREFIX@/etc/proot-distro" ]; then
		return
	fi
	cd "@NEOTERM_PREFIX@/etc/proot-distro"
	find . -mindepth 1 -maxdepth 1 -iname "*.sh" | cut -d/ -f2 | sed -E 's/(.*)\..*/\1/g'
}

_ls_installed_dists() {
	if [ ! -e "@NEOTERM_PREFIX@/var/lib/proot-distro/installed-rootfs" ]; then
		return
	fi
	cd "@NEOTERM_PREFIX@/var/lib/proot-distro/installed-rootfs"
		...
		case "${words[1]}" in
			...
			login|sh)
				if [ $prev = "--user" ]; then
					...
					COMPREPLY=($(compgen -W "${pd_installed_dists} --user --fix-low-ports --isolated --neoterm-home --shared-tmp --bind --no-link2symlink --no-sysvipc --no-kill-on-exit --kernel" -- "$cur"))
				fi
			;;
			remove|rm)
